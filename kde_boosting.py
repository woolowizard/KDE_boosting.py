...
# kernel gaussiano standard
def gaussian_kernel(u):
    return (1/np.sqrt(2*np.pi)) * np.exp(-0.5 * u**2)

# compute estimation of density functions
def compute_f(h, x, weights, index):
    return (weights[index]/h) * gaussian_kernel((x-X[index])/h)

def KDE_boosting(x, log=False):
    ...
    for m in range(1, M+1):
        ...
        for i in range(n):
            if y[i] == 1:
                f1x_est += compute_f(h, x, weights, index=i)
            elif y[i] == -1:
                f2x_est += compute_f(h, x, weights, index=i)
        ...
        for i in range(n):
            ...
            if y[i] == 1:
                f1_x_i += compute_f(h, X[i], weights, index=i)
            elif y[i] == -1:
                f2_x_i += compute_f(h, X[i], weights, index=i)
